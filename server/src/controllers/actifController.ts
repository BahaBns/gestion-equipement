...
const storage = multer.diskStorage({
  destination: function (req, file, cb) {
    const uploadDir = path.join(process.cwd(), "uploads/actifs");
 ...
 */
export const createActif = async (
  req: Request,
  res: Response
): Promise<void> => {
  try {
    ...
    console.log(
      "Using database:",
      (req as any).user?.selectedDatabase || "undefined"
    );
    ...
    if (useMultipleSuppliers) {
      const totalAllocated = supplierAllocations.reduce(
        (sum: number, allocation: { quantity: any }) =>
          sum + parseInt(allocation.quantity || 0, 10),
        0
      );
      ...
      for (const allocation of supplierAllocations) {
        ...
        const supplierExists = await prisma.fournisseur.findUnique({
    ...
    let parsedWarrantyEnd: Date | null = null;
    if (warrantyEnd) {
    ...
    const actifTypeObj = await prisma.actifType.findUnique({
      where: { actifTypeId },
    });
    ...
    if (modeleId) {
      const modeleObj = await prisma.modele.findUnique({
        where: { modeleId },
      });
    ...
    if (!useMultipleSuppliers && fournisseurId) {
      const fournisseurObj = await prisma.fournisseur.findUnique({
        where: { fournisseurId },
      });
    ...
    const existingActifWithSerial = await prisma.actif.findUnique({
      where: { serialNumber },
    });
    ...
    const newActif = await prisma.$transaction(async (prismaClient) => {
      // Create the new Actif
      const createdActif = await prismaClient.actif.create({
        data: {
          actifId,
          serialNumber,
          actifType,
      ...
      if (useMultipleSuppliers && supplierAllocations.length > 0) {
        // Multiple suppliers
        for (const allocation of supplierAllocations) {
          await prismaClient.actifFournisseur.create({
            data: {
    ...
    const transformedActif = {
      ...completeActif,
      employees:
        completeActif?.employees.map((ea) => ({
          ...ea.employee,
          quantity: ea.quantity,
          assignedAt: ea.assignedAt,
        })) || [],
      suppliers:
        completeActif?.fournisseurs.map((af) => ({
          fournisseurId: af.fournisseur.fournisseurId,
          name: af.fournisseur.name,
          email: af.fournisseur.email,
          phone: af.fournisseur.phone,
...
export const updateActif = async (
  ...
  try {
    ...
    if (warrantyEnd !== undefined) {
      ...
      if (
        parsedWarrantyEnd instanceof Date &&
    ...
    if (marqueId) {
      const marqueObj = await prisma.marque.findUnique({
        where: { marqueId },
      });
    ...
    if (useMultipleSuppliers === "true" && supplierAllocations) {
      try {
        ...
        if (parsedQuantity) {
          const totalAllocated = parsedSupplierAllocations.reduce(
            (sum: number, allocation: { quantity: any }) =>
              sum + parseInt(allocation.quantity || 0, 10),
            0
          );
    ...
    // Start a transaction to handle multiple operations
    const updatedActif = await prisma.$transaction(async (prismaClient) => {
      const updateData: any = {};

      // Add basic fields if provided
      if (actifType) updateData.actifType = actifType;
      if (serialNumber) updateData.serialNumber = serialNumber;
      if (actifTypeId) updateData.actifTypeId = actifTypeId;
      ...
      // For marque/marqueId
      if (marqueId) updateData.marqueId = marqueId;

      // For modele/modeleId
      if (modeleId) updateData.modeleId = modeleId;

      // Update the actif with the prepared data
      const updated = await prismaClient.actif.update({
        where: { actifId },
        data: updateData,
      });
      ...
      if (
        useMultipleSuppliers === "true" &&
        parsedSupplierAllocations.length > 0
      ) {
        // Delete current supplier relations
        await prismaClient.actifFournisseur.deleteMany({
          where: { actifId },
        });
      ...
      if (specification) {
        const existingSpec = await prismaClient.specification.findUnique({
          where: { actifId },
        });

        if (existingSpec) {
          // Update existing specification
          await prismaClient.specification.update({
            where: { actifId },
            data: {
              ram:
                specification.ram !== undefined
                  ? specification.ram
                  : existingSpec.ram,
              cpu:
                specification.cpu !== undefined
                  ? specification.cpu
                  : existingSpec.cpu,
              stockage:
                specification.stockage !== undefined
                  ? specification.stockage
                  : existingSpec.stockage,
              graphique:
                specification.graphique !== undefined
      ...
      if (req.body.deletedAttachmentIds) {
        try {
          ...
          if (Array.isArray(deletedIds) && deletedIds.length > 0) {
            ...
            const attachmentsToDelete = await prismaClient.attachment.findMany({
              where: {
      ...
      return await prismaClient.actif.findUnique({
        where: { actifId },
        include: {
          actiftype: true,
          status: true,
    ...
    const transformedActif = {
      ...updatedActif,
      employees: updatedActif?.employees.map((ea) => ({
        ...ea.employee,
        quantity: ea.quantity,
        assignedAt: ea.assignedAt,
      })),
      suppliers: updatedActif?.fournisseurs.map((af) => ({
        fournisseurId: af.fournisseur.fournisseurId,
        name: af.fournisseur.name,
        email: af.fournisseur.email,
        phone: af.fournisseur.phone,
 ...
 */
export const deleteActif = async (
  req: Request,
  res: Response
): Promise<void> => {
  try {
    ...
    await prisma.$transaction(async (prismaClient) => {
      ...
      await prismaClient.specification.deleteMany({
        where: { actifId },
      });

      await prismaClient.actifHashtag.deleteMany({
        where: { actifId },
      });
      ...
      await prismaClient.actif.delete({
        where: { actifId },
      });
    });
...
export const assignActifsToEmployee = async (
  ...
  try {
    console.log("Selected database:", (req as any).user?.selectedDatabase);
    ...
    const assignedActifs = await prisma.actif.findMany({
    ...
    const results = await prisma.$transaction(async (prismaClient) => {
      ...
      for (let i = 0; i < actifIds.length; i++) {
        ...
        if (
          ...
          await prismaClient.employeeActif.delete({
            where: {
              employeeId_actifId: {
                employeeId,
                actifId,
          ...
          if (isReserved) {
            ...
            for (const token of pendingTokens) {
              if (
                ...
                const updatedActifIds = token.actifIds.filter(
                  (id) => id !== actifId
                );
                await prismaClient.assignmentToken.update({
                  where: { tokenId: token.tokenId },
        ...
        if (reason) {
          console.log(
            `Actif ${actifId} removed from employee ${employeeId}. Reason: ${reason}`
          );
 ...
 */
export const getActifById = async (
  req: Request,
  res: Response
): Promise<void> => {
  try {
 ...
 */
export const updateActifWarranty = async (
  req: Request,
  res: Response
): Promise<void> => {
  try {
 ...
 */
export const getActifsByActifType = async (
  req: Request,
  res: Response
): Promise<void> => {
  try {
 ...
 */
export const getActifBySerialNumber = async (
  req: Request,
  res: Response
): Promise<void> => {
  try {
 ...
 * Get expiring actifs
 */
export const getExpiringActifs = async (
  req: Request,
  res: Response
): Promise<void> => {
  try {
    ...
    const expiringActifs = await prisma.actif.findMany({
      where: {
        warrantyEnd: {
          gte: today,
          lte: futureDate,
    ...
    // Transform the data
    const transformedActifs = expiringActifs.map((actif) => ({
      ...actif,
      employees: actif.employees.map((ea) => ({
        ...ea.employee,
        quantity: ea.quantity,
      ...
      daysUntilExpiry: Math.ceil(
        (new Date(actif.warrantyEnd as Date).getTime() - today.getTime()) /
          (1000 * 60 * 60 * 24)
      ),
    }));
 ...
 */
export const updateActifQuantity = async (
  req: Request,
  res: Response
): Promise<void> => {
  try {
